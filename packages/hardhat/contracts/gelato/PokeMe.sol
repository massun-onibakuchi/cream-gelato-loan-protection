...
contract PokeMe is Gelatofied {
    ...
    function createTask(
        ...
        bytes32 task = getTaskId(msg.sender, _execAddress, _execSelector, _useTaskTreasuryFunds, resolverHash);

        require(taskCreator[task] == address(0), "PokeMe: createTask: Sender already started task");
    ...
    function cancelTask(bytes32 _taskId) external {
        require(taskCreator[_taskId] == msg.sender, "PokeMe: cancelTask: Sender did not start task yet");
    ...
    function exec(
        ...
        require(taskCreator[task] == _taskCreator, "PokeMe: exec: No task found");

        (bool success, ) = _execAddress.call(_execData);
        require(success, "PokeMe: exec: Execution failed");

        if (_useTaskTreasuryFunds) TaskTreasury(taskTreasury).useFunds(_feeToken, _txFee, _taskCreator);
    ...
    function getTaskId(
        ...
        return keccak256(abi.encode(_taskCreator, _execAddress, _selector, _useTaskTreasuryFunds, _resolverHash));
    }
    ...
    function getResolverHash(address _resolverAddress, bytes memory _resolverData) public pure returns (bytes32) {
