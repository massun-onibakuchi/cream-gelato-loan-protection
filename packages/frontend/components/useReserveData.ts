...
export function useReserveData(chainId: ChainId, account: string, provider): ReserveData[] {
    ...
    useEffect(() => {
        ...
        async function updateAssets() {
            if (!chainId) return
            try {
                const loanSaver = new Contract(CREAM_GELATO[chainId]['CreamLoanSaverService'], CreamLoanSaverServiceTest.abi, provider) as LoanSaverType
                ...
                setAssets(constants[chainId].map((v: any, i: number) => {
                    return {
